...
void CSledgehammer::Precache()
{
	...
	m_usSledgehammer = PRECACHE_EVENT(1, "events/sledgehammer.sc"); //The fact that this does LITERALLY NOTHING amuses me (see hl_events.cpp)
}
...
bool CSledgehammer::Swing(bool fFirst)
{
	...
	if (tr.flFraction >= 1.0)
	{
		...
		{
			// miss
			m_flNextPrimaryAttack = GetNextAttackDelay(2); //0.5
	...
	else
	{
		...
		// Kamel here yet again, I need to fucking fix plrDmgSledgehammer, which seems to not work at all
		// Kamel here for a final time, I'm pretty sure this is fixed already
		...
		pEntity->TraceAttack(m_pPlayer->pev, gSkillData.plrDmgSledgehammer, gpGlobals->v_forward, &tr, DMG_CRUSH);
		ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);

#endif

		m_flNextPrimaryAttack = GetNextAttackDelay(1); // 0.5, this seems to change the speed of the crowbar
